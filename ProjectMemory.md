...
- **Recent Progress**:
  ...
  - `data-integrity-and-subscriptions-overhaul`: Resolved 5 core room-for-improvement vectors: (1) Eliminated data truncation hazard in `AnalyticsView` total inflow and `SettingsView` CSV export by replacing `state.recentTransactions` (capped at 20) with complete aggregate dataset; (2) Built interactive "Subscriptions & Recurring Bills" management card in `BudgetsView` showing active recurring commitments, monthly burn, next due dates, and cancel action; (3) Added quick date selection chips (`Today`, `Yesterday`, `2d ago`, `1w ago`) in `AddTransactionBottomSheet` enabling backdated logging; (4) Added inline transaction edit capability with `onUpdate` callback and editing dialog in `TransactionDetailBottomSheet`; (5) Added `updateTransaction`, `updateAccount`, and `deleteCategory` in `MoneTrackaDatabase.sq` and repository layers; (6) Synced `preview.html` with interactive subscriptions, date chips, and edit flow; (7) Verified with 100% unit test pass across all modules.
